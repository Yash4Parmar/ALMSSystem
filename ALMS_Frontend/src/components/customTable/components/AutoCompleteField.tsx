...
const AutoCompleteField = ({
	...
	const [value, setValue] = useState<Global.Option | Global.Option[] | null>(multiple ? [] || {} : null);
	const [searchParams, setSearchParams] = useSearchParams();

	const paramKey = `${label}Ids`;
	const newLabel = capitalizeFirstLetter(label) + `${multiple ? "s" : ""}`;

	const handleChange = (_: SyntheticEvent<Element, Event>, newValue: Global.Option[] | Global.Option | null) => {
		...
		<Autocomplete
			...
			renderOption={(props, option) => (
				<Box component='li' {...props}>
					{renderIcon ? <UserProfileAvatar name={option.label} /> : option.label}
				</Box>
			)}
			renderInput={(params) => (
				<TextField
					{...params}
					label={newLabel === "Statuss" ? "Status" : newLabel === "LeaveTypes" ? "Leave Types" : newLabel}
					placeholder={`Select ${newLabel === "Statuss" ? "Status" : newLabel === "LeaveTypes" ? "Leave Types" : newLabel}`}
				/>
			)}
